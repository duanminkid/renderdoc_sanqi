...
def run_tests(test_include: str, test_exclude: str, in_process: bool, slow_tests: bool, debugger: bool, test_timeout: int):
    ...
    log.header("Tests running for SanQi Capture Version {} ({})".format(rd.GetVersionString(), rd.GetCommitHash()))
    ...
    if server is not None:
        ...
        if logfile is not None and os.path.exists(logfile):
            log.inline_file('Replay SanQi Capture log', logfile)
    ...
    if os.path.exists(logfile):
        log.inline_file('{} SanQi Capture log'.format("Host" if server is not None else ""), logfile)
...
def internal_run_test(test_name):
    ...
    for testclass in testcases:
        if testclass.__name__ == test_name:
            ...
            if logfile is not None and os.path.exists(logfile):
                log.inline_file('{} SanQi Capture log'.format("Test" if server is not None else ""), logfile)
